...
denominator and one using the all-LULC mean. In both specifications
weights are constructed from the all-LULC panel dispersion so that the
weighting scheme is held fixed across denominator definitions. The WLS
results are consistent with the mixed-effects results, showing a
positive and statistically significant trend.
...
under either method (p-values 0.13891 and 0.06846).

In the sample ending in 2023 (reported in the
\href{https://everval.github.io/Airborne-Fraction-WLS-Trend/supplementary-preview.html}{Supplementary
Information}), WLS remains significant under both mean definitions
(p-values 0.00059 and 0.00060), while OLS using GCB LULC is only
marginally significant under conventional inference (p-value = 0.09516),
and OLS using the all-LULC mean is not significant (p-value = 0.27109).
...
With \(n_t \ge 2\), the empirical variance estimator is \[
...
\[
\hat a_t = \alpha + \beta t + \varepsilon_t, \quad \varepsilon_t \sim (0, \sigma_t^2), \quad \sigma_t^2 = \operatorname{Var}(\hat a_t).
\]
...
\[
\widehat{\operatorname{Var}(\hat a_t)}=\left(\frac{b_t}{\hat C_t^2}\right)^2 s_{c,t}^2.
\]
